...
module.exports = (config) => {
    ...
    router.post('/cantidadTodo', async (req, res) => {
        const { folios, cantidades, ordenesCompra, numerosParte, nomina } = req.body;
        ...
        try {
            ...
            for (let i = 0; i < folios.length; i++) {
                ...
                const result = await pool.request()
                    ...
                    .query('SELECT * FROM Codigos WHERE Folio = @folio');

                if (result.recordset.length > 0) {
                    const updateQuery = `
                    UPDATE Codigos 
                    SET [Cantidad Entrega] = @cantidad, [Usuario Entrega] = @nomina, 
                        [Orden de Compra] = @ordenCompra, [Numero de Parte] = @numeroParte
                    WHERE Folio = @folio
                `;
            ...
            res.status(200).json({ success: true, message: 'Cantidad de entrega actualizada o insertada correctamente para todos los folios.' });

        } catch (error) {
            console.error('Error al insertar o actualizar cantidad de entrega:', error);
            res.status(500).json({ success: false, message: 'Hubo un error al registrar las cantidades.' });
        } finally {
            sql.close();
        }
    });
